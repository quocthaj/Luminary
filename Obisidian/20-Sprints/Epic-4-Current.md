...
- [ ] Bấm nút "Quiz" ở Workspace left sidebar gọi API `POST /api/tools/[jobId]/quiz`
- [ ] Nếu chưa có cache, Next.js server proxy lên backend Lambda sinh quiz (Gemini Structured Output) gồm 5 câu hỏi và đáp án đúng.
- [ ] Lưu kết quả JSON sinh được vào S3 results bucket tại `results/{jobId}/quiz.json`.
- [ ] Hiển thị Modal trắc nghiệm tương tác, nộp bài tính điểm và phản hồi đúng/sai kèm giải thích trực quan.
- [ ] Hỗ trợ render LaTeX (bằng KaTeX) cho công thức toán trong Quiz Modal.

### Phân chia task
- [ ] **Task 1:** Cấu hình Backend API Gateway route `/job/{jobId}/quiz` (CDK stack + Lambda router).
- [ ] **Task 2:** Xây dựng Lambda Handler xử lý sinh và cache Quiz (`be/lambda/handlers/quiz.ts`) sử dụng Structured Output mode.
- [ ] **Task 3:** Viết Unit tests cho Backend Lambda handler.
- [ ] **Task 4:** Xây dựng Frontend API Route Next.js proxy.
- [ ] **Task 5:** Thiết kế UI nút Quiz và component `QuizModal.tsx` chơi trắc nghiệm hỗ trợ KaTeX.
- [ ] **Task 6:** Viết E2E tests bằng Playwright.
