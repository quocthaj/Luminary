...
export async function GET(
  ...
  const count = req.nextUrl.searchParams.get('count') || '';
  const backendUrl = `${API_BASE}/job/${jobId}/quiz${count ? `?count=${count}` : ''}`;
  console.log(`🧠 [NextJS API Quiz GET] Forwarding request to API Gateway: ${backendUrl}`);
  ...
  try {
    ...
    if (!res.ok) {
      ...
      console.error(`❌ [NextJS API Quiz GET] Backend returned ${res.status}:`, errorData);
      return NextResponse.json(errorData, { status: res.status });
    }
    ...
    console.error('❌ [NextJS API Quiz GET] Error forwarding request:', err);
    return NextResponse.json(
      { error: err.message || 'Internal server error' },
      { status: 500 }
    );
