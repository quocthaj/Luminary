...
Phần Frontend của VietAI Scholar được xây dựng trên framework **Next.js 16 (App Router)** và **React 19**, mang lại hiệu suất render tối ưu kết hợp giữa Server-side Rendering (SSR) và Client-side Hydration. Toàn bộ phần giao diện được lập trình bằng ngôn ngữ TypeScript và định hình phong cách qua **TailwindCSS v4**.
...
1.  **`LandingView`**: Hiển thị trang giới thiệu dịch vụ và các ưu thế của VietAI Scholar.
